...
def get_characters(characters):
    """Функция принимает строку с характеристиками автомобиля. Преобразует её
    в список, разбивая по знакам табуляции. Пытается найти индексы
    соответсвующих характеристик и увеличив их на единицу получает значения
    этих характеристик. Если индекс не найден в значение характеристики
    записывается 'None'. Функция возвращает полученные значения характеристик
    в виде кортежа.
    Args:
    ...
    # Пытаемся получить значение категории характеристики выполняя поиск
    # индекса наименования категории и присваивая соответствующей переменной
    # значение следующего за наименованием категории характеристики элемента.
    # Если категория не обнаружена - переменной присваивается значение 'None'
    try:
...
# Функция для разделения признака 'engine' на составляющие: мощность,
...
def get_engine(eng_str):
    ...
    # В цикле перебираем список полученный разбиением строки по знаку
    # табуляции
    for elem in eng_str.split('/'):
        ...
        # Если поиск по этим условиям не дал результата - значит текущий
        # элемент содержит информацию об объеме двигателя для двигателей
        # внутреннего сгорания или мощность в КВт для электрических двигателей
        else:
            eng_vol = elem.strip()
...
def get_brand(info_str):
    """Функция принимает на вход строку из поля info. Разбивает её на
    список по знаку табуляции. Первый элемент этого списка разбивается в
    список по знаку пробела. Функция сравнивает первый элемент полученного
    списка с ключами заранее определнного словаря, содержащего составные
    наименования марок машин. Если ключ найден - возвращается значение из
    словаря, если нет - первый элемент полученного списка.
...
def get_model(data):
    """Функция принимает на вход строку датафрейма. Преобразует поле info в
    список по знаку табуляции. Первый элемент этого списка разбивается на
    список по знаку пробела и заносится в переменную info. Функция создает
    списки brand и generation разбив однноименные поля строки датафрейма
    по знаку пробела. Далее создается список model содержащий элементы
    отсутствующие в списках brand и generation. Функция возвращает объединный
    через знак пробела в строку итоговый список model.
    ...
    # Создаем списки разбив значения строк с маркой и поколением автомобиля по
    # знаку пробела
    brand = data['brand'].split(' ')
    generation = data['generation'].split(' ')

    # В данных есть наименование модели, которое полностью присутствует в
    # наименовании поколения. Такой случай всего один, обработаем его отдельно
    ...
    # которые отсутствуют в списке brand
        ...
        # Переопределяем список  model оставляя только те элементы, которые
        # отсутствуют в списке generation
        model = [item for item in model if item not in generation]

        # Возвращаем строку с наименованием модели, объединив элементы списка
        # model через знак пробела
        return ' '.join(model)
...
def get_id(head_str):
    ...
    Преобразует её в список по знаку табуляции. В цикле перебираем список и
    выполняем проверку каждого первого элемента для текущего значения. Если
    оно является знаком № - значит это тот элемент списка, который мы ищем.
...
def restore_mileage(df):
    ...
    с пробегом. Если пробег (признак 'mileage') не указан - строка из поля
    'info' разбивается на список по знаку табуляции. Элементы списка
    перебираются в цикле. На каждой итерации цикла выполняется поиск подстроки
    'км'. В случае нахождения функция возращает этот элемент списка. Если
    пробег был указан в признаке 'mileage' функция вернет это значение.
    ...
    if df['mileage'] == 'None':
        ...
        for elem in df['info'].split('\n'):
            ...
            # Каждый элемент разбиваем в список по знаку пробела и ищем
            # подстроку 'км', а также отсутствие подстроки "Заряд" т.к. для
            # электромобилей указывается еще запас хода в км
...
def get_cat_options(options_str):
    ...
    фиксированные наименования категорий опций и сами опции. Функция разбивает
    строку на список по знаку табуляции и создает список с индексами
    наименований категорий опций из исходного списка. Далее в цикле
    перебирается список индексов и для соответствующей категории опции в
    заранее определенную переменную записываются сами опции через срез
    исходного списка по полученным индексам. Функция возвращает значения
    переменных (сами опции) в виде кортежа

    Args:
        options_str (str): исходная строка содержащая катеогрии опций и сами
        опции
    ...
    if options_str is np.nan:
        ...
        # Записываем найденные индекс и наименование категории в отдельный
        # список
        ...
        for num, elem in enumerate(ls_ind):
            ...
            # в соответствующую переменную с помощью среза для случаев
            # нахождения категории в теле или в конце списка опций. И удаляем
            # лишний символ '•'.
            if elem[1] == 'Мультимедиа':
                try:
                    option_media = [i for i in options_ls[elem[0] +
                                    1:ls_ind[num + 1][0]] if i != '•'
                                    ]
                except BaseException:
                    option_media = [
                        i for i in options_ls[elem[0] + 1:] if i != '•']

            elif elem[1] == 'Салон':
                try:
                    option_interior = [i for i in options_ls[elem[0] +
                                       1:ls_ind[num + 1][0]] if i != '•'
                                       ]
                except BaseException:
                    option_interior = [
                ...
                try:
                    option_exterior = [i for i in options_ls[elem[0] +
                                       1:ls_ind[num + 1][0]] if i != '•'
                                       ]
                except BaseException:
                    option_exterior = [
                ...
                try:
                    option_visibility = [i for i in options_ls[elem[0] +
                                         1:ls_ind[num + 1][0]] if i != '•'
                                         ]
                except BaseException:
                    option_visibility = [
                ...
                try:
                    option_safety = [i for i in options_ls[elem[0] +
                                     1:ls_ind[num + 1][0]] if i != '•'
                                     ]
                except BaseException:
                    option_safety = [
                ...
                try:
                    option_comfort = [i for i in options_ls[elem[0] +
                                      1:ls_ind[num + 1][0]] if i != '•'
                                      ]
                except BaseException:
                    option_comfort = [
                ...
                try:
                    option_protection = [i for i in options_ls[elem[0] +
                                         1:ls_ind[num + 1][0]] if i != '•'
                                         ]
                except BaseException:
                    option_protection = [
                ...
                try:
                    option_other = [i for i in options_ls[elem[0] +
                                    1:ls_ind[num + 1][0]] if i != '•'
                                    ]
                except BaseException:
                    option_other = [
        ...
        return option_media, option_interior, option_exterior, \
...
def get_safety(cat_options):
    ...
    В цикле перебираются элементы полученного списка. На каждой итерации
    выполняется проверка на соответствие конкретной опции. В случае нахождения
    соответствия текущий элемент записывается в заранее определенную
    переменную (по умолчанию все переменные имеют значение 'Не указано'),
    описывающую конкретную опцию. Функция возвращает кортеж с полученными
    значениями переменных (опций).
    Args:
    ...
    for elem in cat_options:
        ...
        if elem != 'Не указано':
            ...
            elif (elem == 'Подушки безопасности боковые') or \
                 (elem == '•Подушки безопасности боковые'):
                airbags_side = elem

            elif re.search("Подушки безопасности боковые задние", elem):
                airbags_s_rear = elem

            elif re.search("Подушка безопасности для защиты коленей водителя",
                           elem
                           ):
                airbags_knees_driv = elem

            elif re.search("Подушка безопасности для защиты коленей пассажира",
                           elem
                           ):
            ...
            elif re.search('предупреждения о выезде из полосы',
                           elem.lower()
                           ):
            ...
            elif re.search('стабилизации рулевого управления|vsm',
                           elem.lower()
                           ):
                vsm = elem

            elif re.search('распределения тормозных усилий|bas|ebd',
                           elem.lower()
                           ):
                bas_ebd = elem
    ...
    return airbags_driv, airbags_pass, airbags_side, airbags_s_rear, \
        airbags_knees_driv, airbags_knees_pass, airbags_wind, \
        collision_warn, collision_avoid, lane_depart_warn, lane_holds, \
        traffic_jam_assist, driv_fatigue_sens, road_sign_recogn, asr, vsm, \
        bas_ebd, hill_start_assist, descent_assist, blind_spot_monitor, \
        assist_revers_parking, night_vision, isofix_rear, isofix_front, \
        pressure_sensor, abs_sys, esp, lockout, era_glonass, armored_body
...
def get_visibility(cat_options):
    ...
    return laser_headlights, led_headlights, xenon_headlights, \
        heat_wind_wipers, heat_wind, heat_mirrors, heat_wind_washer, \
        anti_frog, adaptive_light, rain_sens, light_sens, day_lights, \
        auto_headlight_corr, auto_high_beam, headlight_washer
...
def get_media(cat_options):
    ...
    соответствия текущий элемент записывается в заранее определенную
    переменную (по умолчанию все переменные имеют значение 'Не указано'),
    описывающую конкретную опцию. Функция возвращает кортеж с
    ...
    for elem in cat_options:
        ...
        if elem != 'Не указано':
            ...
            # При точом поиске будем искать дополнительный вариант с
            # символом "•"
            if elem == 'Премиальная аудиосистема' or \
               elem == '•Премиальная аудиосистема':
                premium_audio = elem
    ...
    return premium_audio, audio, audio_prep, ru_menu, lcd_screen, \
        media_rear_pass, remote_control, wireless_charg, usb, navi_sys, \
        voice_control, android_auto, carplay, yandex_auto, aux, bluetooth, \
...
def get_comfort(cat_options):
    ...
    for elem in cat_options:
        ...
        if elem != 'Не указано':
            ...
            elif (elem == 'Электростеклоподъемники передние') or \
                 (elem == '•Электростеклоподъемники передние'):
                e_wind_font = elem

            elif (elem == 'Электростеклоподъемники задние') or \
                 (elem == '•Электростеклоподъемники задние'):
                e_wind_rear = elem

            elif elem == 'Круиз-контроль' or elem == '•Круиз-контроль':
                cruise_control = elem

            elif (elem == 'Адаптивный круиз-контроль') or \
                 (elem == '•Адаптивный круиз-контроль'):
                a_cruise_control = elem

            elif (elem == 'Регулировка руля по высоте') or \
                 (elem == '•Регулировка руля по высоте'):
                s_wheel_adj_height = elem

            elif (elem == 'Регулировка руля по вылету') or \
                 (elem == '•Регулировка руля по вылету'):
                s_wheel_adj_reach = elem

            elif (elem == 'Электрорегулировка руля') or \
                 (elem == '•Электрорегулировка руля'):
                s_wheel_adj_elect = elem

            elif (elem == 'Рулевая колонка с памятью положения') or \
                 (elem == '•Рулевая колонка с памятью положения'):
                s_wheel_adj_mem = elem

            elif elem == 'Усилитель руля' or elem == '•Усилитель руля':
                amplifier_wheel = elem

            elif (elem == 'Активный усилитель руля') or \
                 (elem == '•Активный усилитель руля'):
                a_amplifier_wheel = elem

            elif (elem == 'Парктроник передний') or \
                 (elem == '•Парктроник передний'):
                front_park = elem

            elif elem == 'Парктроник задний' or elem == '•Парктроник задний':
                rear_park = elem

            elif (elem == 'Система автоматической парковки') or \
                 (elem == '•Система автоматической парковки'):
                auto_park = elem
    ...
    return climate_1_zone, climate_2_zone, climate_m_zone, air_cond, cam_360, \
        cam_front, cam_rear, e_wind_font, e_wind_rear, cruise_control, \
        a_cruise_control, s_wheel_adj_height, s_wheel_adj_reach, \
        s_wheel_adj_elect, s_wheel_adj_mem, amplifier_wheel, \
        a_amplifier_wheel, front_park, rear_park, auto_park, head_up_disp, \
        drive_mode, remote_start, trunk_without_hands, multifunc_wheel, \
        on_board_comp, electr_instr_panel, keyless_entry_sys, engine_button, \
        prog_pre_heater, electric_trunk, electr_fold_mirror, mirror_memory, \
        paddle_shifters, pedal_assembly, start_stop_sys, electric_mirror, \
        glove_box, door_closer, lighter_ashtray
...
def get_exterior(cat_options):
    ...
    return d_type_steel, d_type_light, d_size_12, d_size_13, d_size_14, \
        d_size_15, d_size_16, d_size_17, d_size_18, d_size_19, d_size_20, \
        d_size_21, d_size_22, d_size_23, d_size_24, d_size_25, d_size_26, \
        d_size_27, d_size_28, two_paint, body_kit, roof_rails, airbrush, \
        moldings


# Функция для извлечения опций из категории "защита от угона"
def get_protection(cat_options):
    """Функция принимает на вход список с опциями из категории
    "защита от угона". В цикле перебираются элементы полученного списка.
    На каждой итерации выполняется проверка на соответствие конкретной опции.
    В случае нахождения соответствия текущий элемент записывается в заранее
    определенную переменную (по умолчанию все переменные имеют значение
    'Не указано'), описывающую конкретную опцию. Функция возвращает кортеж с
    полученными значениями переменных (опций).
    Args:
        cat_options (list): список с опциями конкретной категории
    ...
    for elem in cat_options:
        ...
        if elem != 'Не указано':
            ...
            elif (elem == 'Сигнализация с обратной связью') or \
                 (elem == '•Сигнализация с обратной связью'):
...
def get_interior(cat_options):
    ...
    for elem in cat_options:
        ...
        if elem != 'Не указано':
            ...
            elif (elem == 'Кожа (материал салона)') or \
                 (elem == '•Кожа (материал салона)'):
                in_mat_leather = elem
            ...
            elif re.search('регулировка сиденья водителя по высоте',
                           elem.lower()
                           ):
                seat_drive_h_adj = elem

            elif re.search('регулировка передних сидений по высоте',
                           elem.lower()
                           ):
                seat_front_h_adj = elem

            elif re.search('электрорегулировка сиденья водителя',
                           elem.lower()
                           ):
                seat_drive_e_adj = elem

            elif re.search('электрорегулировка передних сидений',
                           elem.lower()
                           ):
            ...
            elif re.search('сиденье водителя с поясничной поддержкой',
                           elem.lower()
                           ):
                drive_lumbar_sup = elem

            elif re.search('передние сиденья с поясничной поддержкой',
                           elem.lower()
                           ):
            ...
            elif re.search('солнцезащитные шторки в задних дверях',
                           elem.lower()
                           ):
                shades_rear_doors = elem

            elif re.search('солнцезащитная шторка на заднем стекле',
                           elem.lower()
                           ):
    ...
    return in_color_light, in_color_dark, in_mat_leather, in_mat_combi, \
        in_mat_artif_l, in_mat_fabric, in_mat_velour, in_mat_alcantara, \
        seat_drive_h_adj, seat_front_h_adj, seat_drive_e_adj, \
        seat_front_e_adj, seat_rear_e_adj, drive_seat_mem, front_seat_mem, \
        drive_lumbar_sup, front_lumbar_sup, front_seat_heat, rear_seat_heat, \
        front_seat_vent, rear_seat_vent, seat_count_2, seat_count_4, \
        seat_count_5, seat_count_6, seat_count_7, seat_count_8, \
        seat_count_9, sport_seat, luke, panoramic, massage_seat, \
        heated_wheel, leather_wheel, leather_lever, black_ceiling, \
        rear_armrest, third_row_seats, fold_rear_seat, backrest_fold, \
        fold_table, in_light, f_center_armrest, rear_headrest, tinted_glass, \
        shades_rear_doors, shades_rear_window, pedal_pads, door_sills
...
def get_other(cat_options):
    ...
    return active_suspension, sport_suspension, air_suspension, \
        full_spare_wheel, spare_wheel, towbar, crankcase_prot


# Функция для подсчета количества опций автомомбиля, указанных в объявлении
def get_count_options(df):
    """Функция принимает на вход строку датасета. Удаляет столбцы с
    категориями опций. В цикле перебирает значения опций. Если текущее
    значение опции не является ни нулем  ни "Не указано" - увеличивает
    счетчик опций на единицу. Функция возвращает количество указанных в
    объявлении опций автомобиля.
...
# функция реализующая обработку выбросов методом межквартильных интервалов
# (метод Тьюки)
def outliers_irq(df, feature):
    """Функция принимает на вход датасет и имя признака, в котором нужно
    провести обработку выбросов. Производится вычисление 1 и 3 квартилей
    для указанных в списке признаков. Находится межкварьтльный интервал как
    разность между 3ей и 1ой квартилями. Нижняя граница допустимых значений
    определяется как разность между 1ой квартилей и 1.5 величиной
    межквартильного интервала. Верхняя - как сумма 3ей квартили и 1.5
    величиной межквартильного интервала. К входящему датасету применяется
    две маски. Для получения выбросов - берутся все значения выходящие за
    границы допустимых значений. Для очищенных данных - все значения
    находящиейся внутри границ допустимых значений. Функция возвращает
    два датасета: первый - выбросы, второй - оцищенные данные
...
def get_score(estimator, X, y):
    ...
    ответов в логарифмическом масштабе. Делает предсказание с помощью
    полученной модели и приводит их в нормальный масштаб. Производит расчет
    средней абсолютной процентной ошибки и возвращает результат.

    Args:
        estimator (object type that implements the 'predict' method): модель,
        обученная на логарифмированных правильных ответах.
        X ({array-like, sparse matrix} of shape (n_samples, n_features)):
        матрица наблюдений.
        y (array-like of shape (n_samples,)): вектор правильных ответов в
    ...
    score = metrics.mean_absolute_percentage_error(
        y_true=np.exp(y) - 1, y_pred=pred) * 100
    return score
...
def plot_learn_curve(model, X, y, cv, scoring, ax=None, title=''):
    ...
    Args:
        model (object type that implements the 'fit' method): модель,
        подвергаемая проверке.
        X ({array-like, sparse matrix} of shape (n_samples, n_features)):
        матрица наблюдений.
        y (array-like of shape (n_samples,)): вектор правильных ответов.
        cv (int or cross-validation generator): кросс-валидатор из бибилиотеки
        sklearn или количество фолдов для разбиения выборки. По умолчанию
        используется кросс-валидация k-fold на 5 фолдах.
        scoring (str or callable): название метрики в виде строки или функция
        для ее вычисления.
        ax (matplotlib Axes): оси для построения кривой обучения. По
        умолчанию - None.
        title (str, optional): Наименование модели для которой строится график.
        По умолчанию - ''.
    """
    # Вычисляем координаты для построения кривой обучения
    tr_size, tr_score, val_score = model_selection\
...
def get_score_mse(estimator, X, y):
    ...
    правильных ответах модель, матрицу наблюдений и вектор правильных
    ответов в логарифмическом масштабе. Делает предсказание с помощью
    полученной модели и приводит их в нормальный масштаб. Производит
    расчет средней квадратичной ошибки и возвращает результат.

    Args:
        estimator (object type that implements the 'predict' method):
        модель, обученная на логарифмированных правильных ответах.
        X ({array-like, sparse matrix} of shape (n_samples, n_features)):
        матрица наблюдений.
        y (array-like of shape (n_samples,)): вектор правильных ответов в
        логарифмическом масштабе.
