...
if __name__ == '__main__':
    ...
    # в котором передаем строку с признаками (по сути - заполненную анкету
    # с сайта auto.ru)
    ...
    if r.status_code == 200:
        ...
        print(f'Метка данных из тестового набора признаков: '
              f'{X.iloc[random_row, 0]}')
        print(f"Для полученных данных предсказанная моделью "
              f"цена автомобиля составляет: "
              f" {r.json()['prediction']} рублей")

    else:

        # если запрос завершён с кодом, отличным от 200, выводим содержимое
        # ответа
        print(r.text)
